  ...
  instantiateContract() {
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
      simpleStorage.deployed().then((instance) => {
        ...
        console.log('hashes in blockchain 64',ipfsHash+","+this.state.ipfsHash)
        return this.setState({ipfsHash: ipfsHash +","+this.state.ipfsHash})
      })
    })
  }
  ...
  renderData(arr){
    const numbers = arr;
     const listItems = numbers.map((number) =>
      ...
      <img src={`https://ipfs.io/ipfs/${number}`} alt=""/>
    );
    return (
      <ul>{listItems}</ul>
    );
